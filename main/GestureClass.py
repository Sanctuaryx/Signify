...
class Gesture:
    def __init__(self, roll_threshold, roll_range, 
                 pitch_threshold, pitch_range, 
                 yaw_threshold, yaw_range, 
        ...
        self.finger_flex_thresholds = finger_flex_thresholds
    ...
    def _classify_flex(self, fingers):
        """Classify a flex value as 'flexed', 'semi', or 'extended'."""
        return [fingers[i] == self.finger_flex_thresholds[i] for i in range(len(fingers))] 
...
gesture_a = Gesture(
    ...
    finger_flex_thresholds=([30, 30, 30, 30, 30])  # Example thresholds
)
